...
### Additional regression review — 2026-09-05 (updated 2026-09-09)

Added nine tests for Unicode byte limits and round-tripping, proposal modes/no-op outputs, exact-source staleness, duplicate/unindexed/deleted files, excerpt disclosure, invalid UTF-8, search normalization, verification result states, delayed metadata after a new request, and legacy chat decoding. The exact-source regression is now fixed: proposal matching, selection replacement guards, and no-op detection compare UTF-8 bytes rather than canonically equivalent Swift strings. Focused macOS verification passes with zero unexpected failures; the four subprocess lifecycle tests remain skipped by the sandboxed test host and require a direct unsandboxed macOS harness before release.
